...
The project aims to improve the geolocation accuracy of MetOp-B/C AVHRR Level-1B imagery using MODIS as a geometrically accurate reference image.

Unlike traditional image registration that warps the image itself, this project focuses on correcting the latitude and longitude information (navigation data) while preserving the original radiometric values.

**Objectives**
Automatically register AVHRR to MODIS.
Estimate local navigation errors.
Generate reliable tie points.
Build a geometric correction model.
Update AVHRR latitude/longitude grids.
Export a corrected GeoTIFF.
Evaluate the achieved geolocation accuracy.
...
Large swath width
Moderate resolution (~1 km)
Spatially varying navigation errors

A single global transformation cannot accurately correct these errors, so a local window-based registration strategy is adopted.

*Chapter 2 – Registration Methodology*
Overall Workflow
AVHRR + MODIS
        │
        ▼
...
Generate Local Windows
        │
        ▼
Locate Corresponding MODIS Windows
        │
        ▼
Window Quality Filtering (upto here , the code is completed.)
        │
        ▼
...
Transformation Modeling
        │
        ▼
Update Lat/Lon Grids
        │
        ▼
Generate Corrected GeoTIFF
        │
        ▼
Accuracy Assessment
Main Registration Strategy
...
Each window is processed independently, allowing correction of local geometric distortions.

*Chapter 3 – Processing Pipeline*

**Stage 1 — Preprocessing**
Read AVHRR and MODIS
Verify CRS
Reproject if necessary
Match spatial resolution
Extract overlapping region
Preserve latitude/longitude grids

**Output:**

Comparable datasets
**Stage 2 — Window Generation**
...
Finds its center
Uses AVHRR latitude/longitude
Locates the approximate MODIS position
**Stage 3 — Window Validation**

Reject poor windows based on:

Cloud percentage
Low texture
Invalid pixels

Accepted windows proceed to registration.

**Stage 4 — Local Registration**

Each accepted window undergoes:
...
dy
Response
**Stage 5 — Tie Point Processing**

Each registered window becomes one tie point.
...
Second-order polynomial refinement

Generate:

Continuous shift surface
**Stage 7 — Geolocation Correction**

Instead of warping image pixels:
...
Original image radiometry remains unchanged.
...
No code modifications are required for parameter changes.
...
Corrected GeoTIFF
Corrected latitude grid
Corrected longitude grid
...
CE95
Statistics
Chapter 5 – Design Decisions and Future Work
Key Design Decisions
Component	Selected Method
Registration -> Local
Matching ->Phase Correlation
Processing Domain ->Frequency
Window Size -> 256×256
Stride -> 128
Shift Estimation -> Subpixel
Outlier Removal -> RANSAC
Transformation -> Affine
Refinement -> Polynomial (optional)
Correction -> Latitude/Longitude update
Radiometry -> Preserved
...
This project presents an automated framework for geolocation refinement of MetOp-B/C AVHRR Level-1B imagery using MODIS as a reference. The workflow combines preprocessing, local window generation, Phase Correlation, subpixel shift estimation, tie-point validation, RANSAC-based outlier rejection, affine transformation modeling, and latitude/longitude correction. By updating navigation metadata instead of warping the image, the framework preserves radiometric integrity while significantly improving spatial accuracy. Its modular design, configuration-driven execution, and quantitative evaluation make it suitable for both research and operational remote sensing applications.
